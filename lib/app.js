...
function wrapText(text, width) {
  ...
  lines.forEach((line) => {
    ...
    if (hasJapanese) {
      // 日本語モード: 文字単位で折り返し（適切な位置で改行）
      wrapByCharacters(trimmedLine, indent, width, wrappedLines);
    } else {
      // 英語モード: 単語単位で折り返し
      wrapByWords(trimmedLine, indent, width, wrappedLines);
    }
  });
...
// 英語モード: 単語単位で折り返し
function wrapByWords(trimmedLine, indent, width, wrappedLines) {
  ...
  words.forEach((word) => {
    ...
    if (getDisplayWidth(testLine) <= width) {
      ...
      while (
        ...
        let splitPos = indent.length;
        let currentWidth = getDisplayWidth(indent);

        for (let i = indent.length; i < currentLine.length; i++) {
          const charWidth = getCharWidth(currentLine[i]);
        ...
        wrappedLines.push(currentLine.substring(0, splitPos));
        currentLine = indent + currentLine.substring(splitPos);
      }
    }
  });
...
// 日本語モード: 文字単位で適切な位置で折り返し
function wrapByCharacters(trimmedLine, indent, width, wrappedLines) {
  let currentLine = indent;
  let i = 0;
